...
import { MdGpsFixed, MdInfo, MdNavigation, MdCheck, MdClose, MdMyLocation, MdAdd, MdDelete, MdSave, MdCancel } from 'react-icons/md';
import { WiRaindrop } from 'react-icons/wi';

const MapCard2D = ({ onWaypointsUpdate, waypoints: propWaypoints, isViewOnly = false, isLoading = false, children, ...rest }) => {
  ...
  const handleConfirmWaypoint = () => {
    ...
    try {
      ...
      const waypointGeometry = new THREE.ConeGeometry(0.2, 0.4, 3);
      const waypointMaterial = new THREE.MeshBasicMaterial({
        color: isSprayingEnabled ? 0x0066cc : 0x00ff00, // Blue if spraying enabled, green if not
        transparent: true,
        opacity: 0.7
      });
      const waypointCone = new THREE.Mesh(waypointGeometry, waypointMaterial);
      ...
      // Add user data to identify this as a waypoint and store sprayer state
      waypointCone.userData.isWaypoint = true;
      waypointCone.userData.waypointIndex = waypointIndex;
      waypointCone.userData.sprayerEnabled = isSprayingEnabled;
      sceneRef.current.add(waypointCone);
      ...
      const newWaypoint = {
        ...
        sprayerEnabled: isSprayingEnabled
      };
  ...
  const handleWaypointClick = (index) => {
    ...
    // Load the sprayer state
    setIsSprayingEnabled(waypoint.userData.sprayerEnabled || false);
  ...
  const handleConfirmEdit = () => {
    ...
    // Update sprayer state and color
    waypoint.userData.sprayerEnabled = isSprayingEnabled;
    updateWaypointColor(waypoint, isSprayingEnabled);
  ...
  const handleCancelEdit = () => {
    ...
    // Restore original sprayer state and color
    const originalSprayerState = waypoint.userData.sprayerEnabled;
    setIsSprayingEnabled(originalSprayerState);
    updateWaypointColor(waypoint, originalSprayerState);
  ...
  useEffect(() => {
    if (propWaypoints) {
      ...
      propWaypoints.forEach((wp, index) => {
        ...
        const waypointGeometry = new THREE.ConeGeometry(0.2, 0.4, 3);
        const waypointMaterial = new THREE.MeshBasicMaterial({
          color: wp.sprayerEnabled ? 0x0066cc : 0x00ff00, // Blue if spraying enabled, green if not
          transparent: true,
          opacity: 0.7
        });
        ...
        waypointMesh.userData.sprayerEnabled = wp.sprayerEnabled;
  ...
  // Add new function to update waypoint color
  const updateWaypointColor = (waypoint, isSprayerEnabled) => {
    if (waypoint && waypoint.material) {
      waypoint.material.color.setHex(isSprayerEnabled ? 0x0066cc : 0x00ff00);
      waypoint.material.needsUpdate = true;
    }
  };
      ...
      <Box position="relative" width="100%" height="100%" minHeight="600px">
        ...
        {showWaypointControls && (
          ...
          >
            ...
            <Button
              onClick={() => setIsSprayingEnabled(!isSprayingEnabled)}
              borderRadius="full"
              colorScheme="blue"
              size="md"
              variant={isSprayingEnabled ? "solid" : "outline"}
            >
              <Icon 
                as={WiRaindrop} 
                w={isSprayingEnabled ? "24px" : "20px"} 
                h={isSprayingEnabled ? "24px" : "20px"}
                color={isSprayingEnabled ? "white" : "blue.500"}
              />
            </Button>
          </Flex>
        )}
